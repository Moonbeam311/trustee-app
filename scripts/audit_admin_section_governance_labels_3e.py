...
required = ["POST-V2-3E SECTION GOVERNANCE LABELS ACTIVE","section-governance-label","ACTIVE OPERATING SURFACE","LEGACY COMPATIBILITY","DUPLICATE ENTRY POINT","PROTECTED SYSTEM CONTROL","Recommended Next Action","Executive Home","Legacy Compatibility Center","Existing Trust Command Cards","Hosted Baseline Seed","Database Backup","System Policy Controls","Security Layer",CERTIFIED_TAG]
missing = [x for x in required if x not in text]
fail += check("governance label markers present", not missing, "all present" if not missing else ", ".join(missing))
fail += check("active labels count", text.count("ACTIVE OPERATING SURFACE") >= 8, "count=" + str(text.count("ACTIVE OPERATING SURFACE")))
